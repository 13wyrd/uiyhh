...
export function LandingPage() {
  ...
  const getRandomPosition = (index: number) => {
    const seed = index * 137.508; // Golden angle approximation for better distribution
    return {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-hidden relative">
      ...
      {floatingEmojis.map((item, index) => (
        <motion.div
          key={item.id}
          className="absolute text-4xl sm:text-5xl opacity-20 pointer-events-none"
          initial={{
        ...
        <div className="max-w-2xl w-full flex flex-col items-center text-center space-y-8">
          ...
          >
            ...
            <img
              src={avatarGif}
              alt="Avatar"
              className="w-72 sm:w-96 md:w-[500px] h-auto"
            />
          </motion.div>
          ...
          >
            <button className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg text-base sm:text-lg bg-pink-600/20 border border-pink-500/30 text-pink-300 hover:bg-pink-600/30 transition-colors max-w-[280px] sm:max-w-xs text-center leading-tight">
              Click below to experience the full video without limits 🤫
            </button>
          </motion.div>

          {/* Pointing fingers */}
          <div className="relative w-full flex justify-center items-center gap-3 sm:gap-4 md:gap-6 mb-4">
            {/* First finger */}
            <motion.div
              className="text-4xl sm:text-5xl md:text-6xl rotate-90"
          ...
          <motion.button
            ...
            className="group relative px-8 sm:px-12 py-4 sm:py-5 rounded-full text-lg sm:text-xl md:text-2xl font-semibold overflow-hidden"
